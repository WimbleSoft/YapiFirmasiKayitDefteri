...
<nav id="secondary">
      <ul>
        ...
        <li class="current"><a href="firma-santiye-fatura.php?id=<?=$scek['id'];?>&firmaid=<?=$scek['firmaid'];?>">Fatura</a></li>		
      </ul>
    </nav>
...
<div>
  ...
  <?php 
	$vericek=$connection->query("select * from santiyeler_fatura where santiyeid='$id'")->fetchAll(PDO::FETCH_ASSOC);
		foreach ($vericek as $vcek)
		{
	?>
  <tr>
    <td width="84" rowspan="4">SAYIN:</td>
    <td colspan="3" rowspan="4"><textarea style="resize:none; display:none;" name="sayin" id="sayin" cols="45" rows="10"><?php 
	$firmacek=$connection->query("select * from firma where id='$firmaid'")->fetchAll(PDO::FETCH_ASSOC);foreach ($firmacek as $fcek){?><?php $text = "<br/><br/><br/>".$vcek['konu']."<br /><br /><br />".$fcek['firmaadi']."<br/><br/><br/>".$scek['ad']." ".$scek['soyad']."<br>";$breaks = array("<br />","<br />","<br>","<br/>");$text = str_ireplace($breaks, "\r\n", $text); ?><?php }?>
<?php echo $text; ?>
    </textarea>
    <?=$vcek['konu'];?>
    <br /><br /><br />
  ...
  <tr>
    <td>Vergi Dairesi</td>
    <td colspan="2"><input   name="vergidairesi" type="text" id="vergidairesi" /></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td colspan="2"><input type="submit" name="fatura" id="fatura" value="Fatura Oluştur" class="button primary submit"/></td>
    </tr>
  <tr>
    <td>Vergi No</td>
    <td colspan="2"><input   name="vergino" type="text" id="vergino" /></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
