...
<nav id="secondary">
      <ul>
		...
		<li class="current"><a href="firma-santiye-teklif.php?id=<?=$scek['id'];?>&firmaid=<?=$scek['firmaid'];?>">Teklif</a></li>
...
<div>
  <section id="content">
  <div>
  <form action="firma-santiye-teklif-kontrol.php" method="post" enctype="multipart/form-data"><input name="firmaid" hidden="hidden" type="text" id="firmaid" value="<?=$scek['firmaid'];?>" />
	<input name="santiyeid" hidden="hidden" type="text" id="santiyeid" value="<?=$scek['id'];?>" />
  <table width="846" border="0">
  <?php 
	$vericek=$connection->query("select * from santiyeler_teklif where santiyeid='$id'")->fetchAll(PDO::FETCH_ASSOC);
		foreach ($vericek as $vcek)
		{
	?>
  <tr>
    <td width="74" height="24">KONU :</td>
    <td colspan="3"><input   name="konu" type="text" id="konu"size="49" value="<?=$vcek['konu']; ?>"/></td>
    <td width="62">&nbsp;</td>
    <td width="144">Düzenleme Tarihi</td>
    <td colspan="2"><input   name="tarih" type="text" id="tarih" value="<?=$tarih = date("Y-m-d");?>" /></td>
  </tr>
  <tr>
    <td>FİRMA:</td>
    <?php 
  ...
  <tr>
    ...
    <td>&nbsp;</td>
    <td><input type="submit" name="teklif" id="teklif" value="Teklifi Kaydet" class="button primary submit"/>
      <input type="button" name="teklif2" id="teklif2" value="Teklifi Yazdır" onClick="window.open('firma-santiye-teklif-yazdir.php?id=<?=$scek['id'];?>&firmaid=<?=$scek['firmaid'];?>','_blank')" class="button primary submit"/></td>
    
    
    <td colspan="2"><input type="submit" name="hakedis" id="hakedis" value="Hakediş Oluştur" class="button primary submit"/></td>
  </tr>
  <tr>
