...
?>
<form class="wymupdate" action="firma-santiye-montajci-avans-duzenle-kontrol.php" enctype="multipart/form-data" method="post" > 
	   <div class="column left">
		<section>
		  <label for="label">
			Montaj Ekibi
...
document.getElementById("toplamodenecek").value=parseFloat(document.getElementById("miktari2").value)+parseFloat(document.getElementById("odenmis").value);
document.getElementById("toplamodenecek").value=parseFloat(document.getElementById("toplamodenecek").value)-parseFloat(document.getElementById("miktari").value);
	...
	<tr>
		<?php
		$vericek=$connection->query("select * from santiyeler_montajcilar_avanslar where id='$avansid'")->fetchAll(PDO::FETCH_ASSOC);
		foreach ($vericek as $vcek)
		{
		?>
        <td><input id="tarihi" name="tarihi" type="text" value="<?=$vcek['tarihi'];?>"   /></td>
		<?php
		...
		<input name="avansid" id="avansid" hidden="hidden" type="text" value="<?=$avansid;?>"/> 
		...
		<input name="miktari2" id="miktari2" hidden="hidden" type="text" value="<?=$vcek['miktari'];?>"/> 
		<input id="santiye_montajci_id" name="santiye_montajci_id" type="hidden" value="<?=$id;?>"/>
        <td><input class="required" id="miktari" name="miktari" type="text" onkeyup="toplamodemehesapla()" value="<?=$vcek['miktari'];?>"/></td>
		...
		<td><input type="submit" class="button primary submit" value="Avans Düzenle" /></td> 
		<?php } ?>	<?php } ?>	
	</tr>
